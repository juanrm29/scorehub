            ...
            <div className="space-y-4">
              <div className="border border-[#1a1a2e] rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div><label className="block text-xs text-[#666] mb-1">Estimasi Nilai Project (IDR)<InfoTooltip content="5: >Rp 3 Miliar\n4: Rp 2M - 3M\n3: Rp 1M - 2M\n2: Rp 500jt - 1M\n1: <Rp 500jt" /></label><input type="number" placeholder="Mis: 1000000000" value={newInput.estimatedValue ?? ''} onChange={e => setNewInput({ ...newInput, estimatedValue: e.target.value ? +e.target.value : undefined })} /></div>
                  <div><label className="block text-xs text-[#666] mb-1">Term Payment (Hari)<InfoTooltip content="Estimasi hari hingga pembayaran lunas:\n5: ≤14 hari (Kontrak/DP besar)\n4: ≤30 hari (SPK/PO termin)\n3: ≤45 hari (Quotation basis)\n2: ≤60 hari (Termin panjang)\n1: >60 hari (Tanpa jaminan)" /></label><input type="number" placeholder="Mis: 30" value={newInput.termPayment ?? ''} onChange={e => setNewInput({ ...newInput, termPayment: e.target.value ? +e.target.value : undefined })} /></div>
                </div>
              </div>
              <div className="border border-[#1a1a2e] rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div><label className="block text-xs text-[#666] mb-1">Legal Documents<InfoTooltip content="Dokumen legal perusahaan (pisah koma):\n5: ≥5 dok (Lengkap)\n4: 4 dokumen\n3: 3 dokumen\n2: 2 dokumen\n1: <2 dokumen\n(Izin, NPWP, CSMS, Rek, SPK)" /></label><input type="text" value={newInput.legalDocuments ?? ''} onChange={e => setNewInput({ ...newInput, legalDocuments: e.target.value ? e.target.value : undefined })} placeholder="Akte, NIB, NPWP" /></div>
                  <div><label className="block text-xs text-[#666] mb-1">Background Media<InfoTooltip content="Kehadiran digital perusahaan:\n5: 3+ media (Web, LinkedIn, IG)\n4: 2 media\n3: 1 media\n1: Tidak ada" /></label><input type="text" value={newInput.backgroundMedia ?? ''} onChange={e => setNewInput({ ...newInput, backgroundMedia: e.target.value ? e.target.value : undefined })} placeholder="Web, Social, News" /></div>
                  <div><label className="block text-xs text-[#666] mb-1">Referensi/Rekomendasi?<InfoTooltip content="Apakah ada referensi dari relasi bisnis?\n5: Ada referensi\n2: Tidak ada referensi\n\nMasuk ke bobot Credibility 30%" /></label><select value={newInput.hasReference === undefined ? '' : newInput.hasReference ? 'yes' : 'no'} onChange={e => setNewInput({ ...newInput, hasReference: e.target.value === '' ? undefined : e.target.value === 'yes' })}><option value="">— Skip —</option><option value="yes">Ya</option><option value="no">Tidak</option></select></div>
                </div>
              </div>
              <div className="border border-[#1a1a2e] rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="block text-xs text-[#666] mb-1">Technical Documents<InfoTooltip content="Kelengkapan dokumen teknis:\n5: ≥3 dok (Ship part, Surat laut, Repair list)\n4: 2 dokumen\n3: 1 dokumen\n1: Kosong" /></label><input type="text" value={newInput.technicalDocuments ?? ''} onChange={e => setNewInput({ ...newInput, technicalDocuments: e.target.value ? e.target.value : undefined })} placeholder="Vessel Spec, Draw" /></div>
                  <div><label className="block text-xs text-[#666] mb-1">Kecepatan Keputusan (Hari)<InfoTooltip content="Berapa hari client memutuskan setelah quotation:\n5: ≤2 hari\n4: 3-5 hari\n3: 5-7 hari\n2: 7-14 hari\n1: >14 hari" /></label><input type="number" min={1} placeholder="Mis: 3" value={newInput.decisionSpeed ?? ''} onChange={e => setNewInput({ ...newInput, decisionSpeed: e.target.value ? +e.target.value : undefined })} /></div>
                </div>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="border border-[#1a1a2e] rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-4 gap-3">
                  <div><label className="block text-xs text-[#666] mb-1">Cancel Order<InfoTooltip content="Jumlah cancel order:\n5: 0 kali (tidak pernah)\n4: 1 kali\n3: 2 kali\n2: 3 kali\n1: > 3 kali" /></label><input type="number" placeholder="0" value={repInput.cancelOrder ?? ''} onChange={e => setRepInput({ ...repInput, cancelOrder: e.target.value ? +e.target.value : undefined })} /></div>
                  <div><label className="block text-xs text-[#666] mb-1">Variance Jadwal (Hari)<InfoTooltip content="Selisih hari dari jadwal (negatif=lebih awal, positif=terlambat):\n5: ≤-3 hari (lebih awal >3 hari)\n4: -1 sd -2 hari (sedikit lebih awal)\n3: 0 hari (tepat jadwal)\n2: +1 sd +3 hari (sedikit terlambat)\n1: >3 hari (terlambat parah)" /></label><input type="number" placeholder="0" value={repInput.scheduleVariance ?? ''} onChange={e => setRepInput({ ...repInput, scheduleVariance: e.target.value ? +e.target.value : undefined })} /></div>
